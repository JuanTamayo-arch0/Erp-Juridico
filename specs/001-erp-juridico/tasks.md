...
- [ ] T001 Initialize repository skeleton and README at repository root (frontend/backend/electron) - `./`
- [ ] T002 [P] Create frontend skeleton using Vite + React + TypeScript in `frontend/` (package.json, tsconfig, basic routes)
- [ ] T003 [P] Create backend skeleton with Django project in `backend/` (settings, WSGI/ASGI, basic app `cases`)
- [ ] T004 [P] Create Electron wrapper skeleton in `electron/` with packaging config and local SQLite stub
- [ ] T005 [P] Add `docker-compose.yml` for local dev: Postgres 15, Redis 7, and MinIO (S3 emulator) - `docker-compose.yml`
- [ ] T006 [P] Add CI workflow (GitHub Actions) to run linters, tests and enforce Constitution Check presence in `specs/` - `.github/workflows/ci.yml`
- [ ] T007 [P] Add project-wide linting and formatting configs: ESLint + Prettier (frontend), ruff/black (backend), editorconfig - `frontend/`, `backend/`
- [ ] T008 [P] Add initial `requirements.txt` (backend) and `package.json` (frontend/electron) with core deps
- [ ] T009 [P] Create `docs/CONSTITUTION.md` referencing `.specify/memory/constitution.md` and link in README
- [ ] T010 [P] Constitution compliance checkpoint: document how the setup satisfies mandatory constitution gates (security, logging, backups, basic tests) - `specs/001-erp-juridico/plan.md`
